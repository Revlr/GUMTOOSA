...
| DATA-007 | DONE | SQLite 등록 종목 벤치마크 선택 표시 | 벤치마크 입력에서 SQLite 등록 종목 전체를 선택할 수 있음 | DATA-006 |
| UX-004 | DONE | 백테스트 옵션 종목 선택 드롭다운화 | 종목/벤치마크 입력이 SQLite 등록 종목 드롭다운으로 표시되고 별도 칩 목록은 제거됨 | DATA-007 |
| UX-005 | DONE | 결과 지표 용어 설명 보강 | CAGR/MDD/Sharpe 라벨에 괄호 설명이 함께 표시됨 | UX-004 |
| UX-006 | DONE | 안정형 포트폴리오 템플릿 5개 구성 | 올웨더, 60/40, 바벨, 영구 포트폴리오, 골든 버터플라이가 전략 템플릿에 표시됨 | UX-005 |
| UX-007 | DONE | 포트폴리오 템플릿 검증 오류 수정 | 안정형 포트폴리오 선택 시 중복 조건, 종목 입력, 객체 오류 메시지 문제가 해소됨 | UX-006 |

## 완료 기록

### 2026-05-09 - UX-007 포트폴리오 템플릿 검증 오류 수정
- 변경: 안정형 포트폴리오 전략은 드롭다운의 단일 종목 값 대신 포트폴리오 목표 비중의 전체 심볼을 백테스트 옵션으로 사용하도록 수정함
- 변경: 포트폴리오의 자동 리밸런싱 매수/매도 조건은 중복 조건 검증에서 제외하고, 화면에는 목표 비중 기반 자동 리밸런싱 요약으로 표시되도록 정리함
- 변경: 백엔드 API 오류 detail이 객체나 배열로 내려와도 `[object Object]` 대신 읽을 수 있는 메시지로 표시되도록 변환 로직을 추가함
- 검증: 정적 파일 문법 검사와 프론트 서버 로드 확인을 진행함
- 참고: 백엔드 계산 로직과 DB 데이터는 변경하지 않음

### 2026-05-09 - UX-006 안정형 포트폴리오 템플릿 5개 구성
- 변경: 전략 템플릿 목록에 `올웨더 전략 포트폴리오`, `60/40 전략 포트폴리오`, `바벨 전략 포트폴리오`, `영구 포트폴리오`, `골든 버터플라이 포트폴리오`가 표시되도록 구성함
- 변경: 바벨 전략 포트폴리오를 추가하고, 각 안정형 포트폴리오 선택 시 목표 비중 기반 매수/매도 조건이 함께 적용되도록 템플릿 필터를 조정함
- 검증: 템플릿 ID, 이름, 자산 비중, 조건 적용 경로가 정적 파일에 반영된 것을 확인함
- 참고: SQLite에 등록된 ETF 심볼만 사용해 기존 백테스트 데이터 흐름과 충돌하지 않게 구성함

### 2026-05-09 - UX-005 결과 지표 용어 설명 보강
- 변경: 백테스트 결과 지표 카드의 `CAGR`, `MDD`, `Sharpe` 라벨에 각각 `연평균 성장률`, `최대 낙폭`, `위험 대비 수익` 설명을 괄호로 추가함
- 검증: 정적 파일에서 변경 문구 반영 여부와 CSS 문법 검사를 확인함
- 참고: 백테스트 계산 로직과 백엔드 API는 변경하지 않음

### 2026-05-09 - UX-004 백테스트 옵션 종목 선택 드롭다운화

- 변경: 백테스트 옵션의 `종목`, `벤치마크`를 input/datalist와 칩 UI 대신 SQLite 등록 종목 전체를 담은 select 드롭다운으로 변경함
- 검증: `종목`, `벤치마크`가 select로 바뀌고 12개 SQLite 등록 심볼이 옵션으로 반영되는 것을 확인했으며, 별도 칩/datalist DOM이 제거됨. `index.html`과 `app.js`가 프론트 서버에서 HTTP 200으로 로드되고 `git diff --check`도 통과함
- 참고: 별도로 보이던 `SQLite DB 등록 종목`, `벤치마크 후보` 영역은 제거함

### 2026-05-09 - DATA-007 SQLite 등록 종목 벤치마크 선택 표시

- 변경: 벤치마크 후보 칩 목록을 추가하고, SQLite 등록 종목 전체(SPY 포함 12개)를 벤치마크 입력값으로 선택할 수 있도록 연결함
- 검증: `dbSymbols`에 SPY 포함 12개 심볼이 반영된 것을 확인했고, `index.html`과 `app.js`가 프론트 서버에서 HTTP 200으로 로드됨. `git diff --check`도 통과함
- 참고: 기존 종목 입력 칩은 SPY 제외 후보를 유지하고, 벤치마크 후보에는 SPY를 포함함
